...
| | 内容 |
|---|---|
| **基本パッケージ** ¥29,800 | 運用指示書 完全版（AGENTS.md / CLAUDE.md）・人格ファイル5枚・記憶の2層運用設計・cron 設計パターン集・独立検証ゲート4本・スキル3本・配線スクリプト |
| **拡張パッケージ** ¥98,000 | 時間軸つき知識グラフ・自律ループ3層キット（発見/実行/独立検証）・read-only rootfs の Docker 常駐環境 |
