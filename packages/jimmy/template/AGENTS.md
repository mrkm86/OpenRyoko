...
**MEMORY.md（長期記憶）は自分から Read しない**。gateway が信頼相手（private web セッション、または `portal.trustedSpeakers` に Slack ID を載せた相手との DM）とのセッションにだけ system prompt へ注入する。共有チャンネルでは誰が話していても注入されない。個人情報を含むため、必要があって自分で Read した場合も、内容を当人以外に出力で出さないこと。
